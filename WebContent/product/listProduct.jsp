	...
	<c:set var="resultPage" value="${ resultPage }"/>
		...
		<form name="detailForm" action="/product/listProduct?menu=${ menu }" method="post">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
					<td align="right"> 
						<select name="searchCondition" class="ct_input_g" style="width:80px"> 
						 	<!-- //trim 안해도 되나? -->
							<option value="0" ${ !empty svo.searchCondition && search.searchCondition == 0 ? "selected" : "" } >상품번호</option>
							<option value="1" ${ !empty svo.searchCondition && search.searchCondition == 1 ? "selected" : "" } >상품명</option>
							<option value="2" ${ !empty svo.searchCondition && search.searchCondition == 2 ? "selected" : "" } >상품가격</option> 
					  	</select>
						<input 	type="text" name="searchKeyword"  value="${ svo.searchKeyword }" class="ct_input_g" 
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
					<tr> 
						<td colspan="11" >
							<select name="sortingCondition" class="ct_input_g" style="width:80px">  
								<option value="0" ${ !empty svo.sortingCondition && search.sortingCondition == 0 ? "selected" : "" }>상품번호순</option>
							...
							<c:if test="${ menu =='manage' }">
								<c:choose>
									...
									<c:when test="${map.prodTranCode=='001' }">
										구매완료
										<a href="/updateTranCodeByProd?prodNo=${map.prodNo}&tranCode=002">배송하기</a>
									</c:when>
